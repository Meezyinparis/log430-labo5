...
def add_order(user_id: int, items: list):
    ...
    try:
        ...
        new_order = Order(
            user_id=user_id, total_amount=total_amount, payment_link=None)
        session.add(new_order)
        session.flush()

        order_id = new_order.id

        new_order.payment_link = request_payment_link(
            new_order.id, total_amount, user_id)
        session.flush()
        ...
        add_order_to_redis(order_id, user_id, total_amount,
                           items, new_order.payment_link)
        return order_id
