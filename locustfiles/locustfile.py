    ...
    def orders(self):
        ...
        if random.randint(1, 10) <= 3:
            ...
            mock_order["items"].append(
                {"product_id": random_id, "quantity": 1})
            mock_order["items"].append(
            ...
            try:
                ...
                if response is None or response.headers is None:
                    self.environment.events.request.fire(
                        ...
                        response_length=0,
                        exception=Exception(
                            "Aucune réponse (erreur ou timeout)")
                    )
                    return
                ...
                if 'application/json' not in content_type:
                    response.failure(
                        f"Erreur : {response.status_code} - Aucun JSON dans la réponse. Message : {response.text}")
                    return
                ...
                if response.status_code == 201:
                    if "order_id" in data:
                        ...
                        response.failure(
                            "Aucun ID renvoyé pour la commande créée")
                else:
                    response.failure(
                        f"Erreur : {response.status_code} - {data.get('error', 'Unknown error')}")
            except ValueError:
                response.failure(
                    f"JSON invalide dans la réponse: {response.text}")
            ...
            try:
                ...
                if response.status_code == 200:
                    if isinstance(data, list):
                        ...
                        response.failure(
                            "Le résultat n'est pas une liste : " + str(data))
                else:
                    response.failure(
                        f"Erreur : {response.status_code} - {data.get('error', 'Unknown error')}")
            except ValueError:
                response.failure(
                    f"Erreur : {response.status_code} - JSON invalide dans la réponse")
